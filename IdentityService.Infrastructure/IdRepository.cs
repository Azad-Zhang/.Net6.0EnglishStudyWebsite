...
namespace IdentityService.Infrastructure
{
    class IdRepository : IIdRepository
    {
        ...
        public async Task<SignInResult> ChangePhoneNumAsync(Guid userId, string phoneNum, string token)
        {
            ...
            {
                await ConfirmPhoneNumberAsync(user.Id,user.PhoneNumber);//确认手机号
        ...
        public async Task<SignInResult> ChangeEmailAsync(Guid userId, string email, string token)
        {
            ...
            if (!changeResult.Succeeded)
            {
                await this.userManager.AccessFailedAsync(user);
                string errMsg = changeResult.Errors.SumErrors();
                this.logger.LogWarning($"{email}ChangeEmailAsync，错误信息{errMsg}");
                return SignInResult.Failed;
            }
            else
            {
                await ConfirmEmailAsync(user.Id, user.Email);//确认手机号
                return SignInResult.Success;
            }
        }
        ...
        {
            return this.userManager.GenerateChangeEmailTokenAsync(user, email);
        }

        public async Task<IdentityResult> SetLockOut(User user, DateTimeOffset? date, bool IsLock)
        {
            var setEnable = await this.userManager.SetLockoutEnabledAsync(user, IsLock);

            var setDate = await this.userManager.SetLockoutEndDateAsync(user, date);
        ...
        public async Task ConfirmPhoneNumberAsync(Guid id, string phoneNum)
        {
            //159357
        ...
        public async Task<(IdentityResult, User?, string? password)> AddAdminUserAsync(string userName, string phoneNum,string email)
        {
        ...
        public async Task ConfirmEmailAsync(Guid id, string email)
        {
            var user = await userManager.Users.FirstOrDefaultAsync(u => u.Id == id);
            if (user == null)
            {
